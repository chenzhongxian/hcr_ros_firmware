...
/*******************************************************************************
* Changelog:
* 	2017-06-11: Update for HCR, Authors: Zhongxian Chen (xyxybot@gmail.com)
* 		- Update parameters for HCR
* 		- Update motor driver for HCR
* 		- Comment out RC100 related code
*******************************************************************************/

#include "hcr_core_config.h"
...
void setup()
{
  ...
  // Setting for motors and communitcations
  Serial2.begin(57600);
  delay(2000);
  cleanEncoder();
...
void controlMotorSpeed(void)
{
  ...
  //   return;
  speedControl((int)lin_vel1, (int)lin_vel2);
}
...
uint8_t getButtonPress(void)
{
  ...
  for (int button_num = 0; button_num < 2; button_num++)
  {
    switch (button_state_num[button_num])
    {
     ...
     case WAIT_SECOND:
       if ((millis()-t_time[button_num]) >= 100)
       {
         if (getPushButton() & (1 << button_num))
         {
...
void testDrive(void)
{
  ...
  if (start_move)
  {
    diff_encoder = TEST_DISTANCE / (WHEEL_CIRCUMFERENCE / ENCODER_TICK); // (Circumference of Wheel) / (The number of tick per revolution)

    if (abs(last_right_encoder - current_tick) <= diff_encoder)
    {
      // goal_linear_velocity  = 0.05 * SCALE_VELOCITY_LINEAR_X;    // BURGER
      goal_linear_velocity  = 0.10 * SCALE_VELOCITY_LINEAR_X;       // HCR, 0.10 m/s
    }
    else
    {
  ...
  {
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (WHEEL_CIRCUMFERENCE / ENCODER_TICK);

    if (abs(last_right_encoder - current_tick) <= diff_encoder)
    {
      goal_angular_velocity= -0.7 * SCALE_VELOCITY_ANGULAR_Z;     // 0.7 rad/s, 0.09626 m/s
    }
    else
    {
